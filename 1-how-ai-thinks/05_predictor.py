...
def predict_next(phrase):
  phrase_tokens = tokenize(phrase.lower())
  context = phrase_tokens[-2] + ' ' + phrase_tokens[-1]
...
def autocomplete(phrase, num_tokens=10):
  result = phrase.lower()
  for _ in range(num_tokens):
    token = predict_next(result)
    if token is None:
      break
    result = result + ' ' + token
  return result

print(predict_next('once upon a'))
print(predict_next('the princess'))
print(predict_next('purple elephant'))

print(autocomplete('once upon'))
print(autocomplete('she had'))
print(autocomplete('the ball'))
